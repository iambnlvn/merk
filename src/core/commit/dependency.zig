...
// Aliases & Extracted Declarations
...
test "validate rejects more than MAX_DEPENDENCIES, accepts exactly MAX_DEPENDENCIES" {
    ...
    try testing.expectError(error.TooManyDependencies, validate(too_many));
...
test "validate rejects a duplicate change_id" {
    ...
    try testing.expectError(error.DuplicateDependency, validate(&deps));
}
...
test "serializeAll/deserializeAllAlloc round-trip several dependencies" {
    ...
    try testing.expectEqual(@as(usize, 1 + 3 * 16), buf.items.len);
...
test "serializeAll rejects a duplicate before writing anything" {
    ...
    var buf: ArrayList(u8) = .empty;
    defer buf.deinit(alloc);
    try testing.expectError(error.DuplicateDependency, serializeAll(&deps, buf.writer(alloc)));
